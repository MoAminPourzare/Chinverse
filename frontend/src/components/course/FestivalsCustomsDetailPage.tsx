...
import { getFestivalsCustomsCourse } from "@/lib/festivalsCustomsCatalog";
...
export default function FestivalsCustomsDetailPage() {
    ...
    const course = getFestivalsCustomsCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("festivals-customs", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="festivals-customs" explorePath="/explore/festivals-customs" eyebrow="فرهنگ و اندیشهٔ چین" countKeys={["lesson_count", "episodes_count"]} countLabel="قسمت" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/festivals-customs" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به آیین‌ها و جشن‌ها</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/festivals-customs" label="بازگشت به فهرست آیین‌ها و جشن‌ها" />
                    ...
                    <section className="mt-4 grid grid-cols-2 items-center gap-2" dir="ltr">
                        <div className="min-w-0 text-center">
                            ...
                            <p className="mt-2 text-[10px] leading-5 text-[#59616c] dark:text-slate-300" dir="rtl">فرهنگ و اندیشهٔ چین</p>
                        ...
                        <div className="relative aspect-square w-full overflow-hidden rounded-[10px] bg-slate-200">
                            <Image src={course.detailCoverPath || course.coverPath} alt={`کاور ${course.title} ${course.subtitle || ""}`} fill sizes="(max-width: 430px) 78vw, 335px" className="object-cover object-[25%_center]" priority />
                        </div>
                    </section>
                </div>
                ...
                <section className="mt-5 space-y-2" aria-label={`قسمت‌های ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/festivals-customs/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                        ...
                        return href ? <Link key={position} href={href} className="block overflow-hidden rounded-[10px] bg-[#e2e5eb] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#155aa6] dark:bg-[#202b3a]" dir="ltr" aria-label={`پخش ${label}: ${topic} (قسمت ${position})`}>{content}</Link>
