...
import { getTeaCultureCourse } from "@/lib/teaCultureCatalog";
...
export default function TeaCultureDetailPage() {
    ...
    const course = getTeaCultureCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("tea-culture", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="tea-culture" explorePath="/explore/tea-culture" eyebrow="فرهنگ چای" countKeys={["lesson_count", "episodes_count"]} countLabel="بخش" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/tea-culture" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به فرهنگ چای</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/tea-culture" label="بازگشت به فهرست فرهنگ چای" />
                ...
                <section className="mt-5 space-y-2" aria-label={`بخش‌های ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/tea-culture/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                        ...
                        return href ? <Link key={position} href={href} className="block overflow-hidden rounded-[10px] bg-[#e2e5eb] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#155aa6] dark:bg-[#202b3a]" dir="ltr" aria-label={`پخش ${label}: ${topic} (بخش ${position})`}>{content}</Link>
