...
import { getHistoricalStoriesCourse } from "@/lib/historicalStoriesCatalog";
...
export default function HistoricalStoriesDetailPage() {
    ...
    const course = getHistoricalStoriesCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("historical-stories", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="historical-stories" explorePath="/explore/historical-stories" eyebrow="فرهنگ و اندیشهٔ چین" countKeys={["lesson_count", "episodes_count"]} countLabel="قسمت" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/historical-stories" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به داستان‌های کهن</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/historical-stories" label="بازگشت به فهرست داستان‌های کهن" />
                    ...
                    <section className="mt-4 grid grid-cols-2 items-center gap-2" dir="ltr">
                        <div className="min-w-0 text-center">
                            <h1 className="text-[15px] font-bold leading-6 text-[#343941] dark:text-white" lang="en">{course.cardTitle || course.title}</h1>
                ...
                <section className="mt-5 space-y-2" aria-label={`قسمت‌های ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/historical-stories/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                        ...
                        const content = (
                            <article className="grid min-h-32 grid-cols-[1fr_40%] gap-2">
                                <div className="flex min-w-0 flex-col px-2.5 py-3 text-left">
                                    ...
                                    <p className="mt-1 break-words font-bold text-[12px] leading-[17px] text-[#343941] dark:text-slate-100" lang="zh">{topic}</p>
