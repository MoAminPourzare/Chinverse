                ...
                <section className="mt-5" aria-labelledby="hsk-lessons-heading">
                    ...
                    <div className="motion-list space-y-2.5">
                        {Array.from({ length: course.lessonCount }, (_, index) => {
                            ...
                            const topic = getHskLessonTopic(course, index);
                            ...
                            >
                                <article className="grid grid-cols-[1fr_34%] gap-3">
                                    <div className="min-w-0 py-1 text-left">
                                        ...
                                        {topic && <p className="mt-1 line-clamp-2 font-serif text-[11px] leading-4 text-[#4b535e] dark:text-slate-200" lang="zh-Hans">{topic}</p>}
                                        <p className="mt-3 text-right text-[9px] font-medium text-[#777f89] dark:text-slate-400" dir="rtl">{publishedLesson ? "آمادهٔ پخش" : hasError ? "وضعیت ویدیو نامشخص" : isLoading ? "در حال بررسی ویدیو…" : "در دست آماده‌سازی"}</p>
                                    </div>
                                    <div className="min-h-[96px] rounded-[10px] border border-white/70 bg-[linear-gradient(45deg,#f8f8f8_25%,transparent_25%),linear-gradient(-45deg,#f8f8f8_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#f8f8f8_75%),linear-gradient(-45deg,transparent_75%,#f8f8f8_75%)] bg-[length:14px_14px] bg-[position:0_0,0_7px,7px_-7px,-7px_0px] group-hover:bg-white dark:border-slate-600 dark:bg-slate-800 dark:group-hover:bg-slate-700" aria-hidden />
                                </article>
                            </Link>
                            );
                        })}
