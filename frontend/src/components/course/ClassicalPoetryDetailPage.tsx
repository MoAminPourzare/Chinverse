...
import { getClassicalPoetryCourse } from "@/lib/classicalPoetryCatalog";
...
export default function ClassicalPoetryDetailPage() {
    ...
    const course = getClassicalPoetryCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("classical-poetry", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="classical-poetry" explorePath="/explore/classical-poetry" eyebrow="فرهنگ و اندیشهٔ چین" countKeys={["lesson_count", "episodes_count"]} countLabel="قسمت" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/classical-poetry" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به شعر و ادبیات کلاسیک</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/classical-poetry" label="بازگشت به فهرست شعر و ادبیات کلاسیک" />
                    ...
                    <section className="mt-4 grid min-h-[160px] grid-cols-2 items-center gap-2" dir="ltr">
                        ...
                        <div className="relative aspect-video w-full overflow-hidden">
                            <Image src={course.detailCoverPath || course.coverPath} alt={`کاور ${course.title} ${course.subtitle || ""}`} fill
                                sizes="(max-width: 430px) 43vw, 187px" className="object-contain" priority />
                        </div>
                    </section>
                </div>
                ...
                <section className="mt-5 space-y-2" aria-label={`قسمت‌های ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/classical-poetry/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                ...
                <details className="mt-5 rounded-[14px] bg-white p-4 text-[#343941] dark:bg-[#18212b] dark:text-slate-100">
                    <summary className="cursor-pointer text-sm font-bold">معرفی مجموعه</summary>
