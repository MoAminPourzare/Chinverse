...
import { getCookingCourse } from "@/lib/cookingCatalog";
...
export default function CookingDetailPage() {
    ...
    const course = getCookingCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("cooking", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="cooking" explorePath="/explore/cooking" eyebrow="هنر و مهارت‌های چینی" countKeys={["lesson_count", "episodes_count"]} countLabel="قسمت" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/cooking" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به آشپزی</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/cooking" label="بازگشت به فهرست آشپزی" />
                    ...
                    <section className="mt-4 grid grid-cols-2 items-center gap-2" dir="ltr">
                        <div className="min-w-0 text-center">
                            ...
                            {course.subtitle && <p className="mt-0.5 text-[11px] leading-5 text-[#343941] dark:text-white" lang="zh-Latn">{course.subtitle}</p>}
                        ...
                        <div className={`relative mx-auto w-full overflow-hidden ${course.portraitCover ? "aspect-[2/3] max-w-[67%]" : "aspect-square rounded-[10px] bg-slate-200"}`}>
                            <Image src={course.coverPath} alt={`کاور ${course.title}`} fill sizes={course.portraitCover ? "(max-width: 430px) 29vw, 125px" : "(max-width: 430px) 43vw, 187px"} className="object-cover" priority />
                        </div>
                    </section>
                </div>

                <section className="mt-5 space-y-2" aria-label={`قسمت‌های ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/cooking/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                        ...
                        const content = (
                            <article className="grid min-h-32 grid-cols-[1fr_40%] gap-2">
                                <div className="flex min-w-0 flex-col px-2.5 py-3 text-left">
                                    ...
                                    <p className={`mt-1 break-words leading-[17px] text-[#343941] dark:text-slate-100 ${course.chineseTopics ? "font-cjk text-[12px]" : "text-[11px] font-bold"}`} lang={course.chineseTopics ? "zh" : undefined}>{topic}</p>
                                ...
                                {thumbnail ? (
                                    <div className="relative my-1.5 mr-1.5 min-h-[116px] overflow-hidden rounded-[8px] bg-slate-200">
                                        <Image src={thumbnail} alt={`تصویر ${label}: ${topic}`} fill sizes="(max-width: 430px) 35vw, 146px" className="object-cover" />
                                    </div>
