...
export default function TopicTalksDetailPage() {
    ...
    const course = getTopicTalksCourse(params?.id);
    const { publishedCourse, isLoading, hasError } = usePublishedPlannedCourse("topic-talks", course);
    ...
    if (!course && /^\d+$/.test(params?.id || "")) {
        return <CourseDetailPage domain="topic-talks" explorePath="/explore/topic-talks" eyebrow="گفتارهای موضوعی" countKeys={["lesson_count", "episodes_count"]} countLabel="گفتار" />;
    }
    if (!course) {
        return (
            <div className="flex min-h-full items-center justify-center bg-[#f7f8fa] p-5 dark:bg-[#10151c]" dir="rtl">
                <div className="rounded-[24px] border border-[#dfe6f0] bg-white p-8 text-center dark:border-slate-700 dark:bg-[#18212b]">
                    ...
                    <Link href="/explore/topic-talks" className="mt-5 inline-flex rounded-[14px] bg-[#155aa6] px-4 py-2.5 text-sm font-black text-white">بازگشت به گفتارهای موضوعی</Link>
                </div>
            </div>
        );
    }
            ...
            <main className="mx-auto w-full max-w-[430px] px-6 py-4">
                <div className="sticky top-0 z-10 -mx-6 bg-[#f7f8fa] px-6 pb-5 dark:bg-[#10151c]">
                    <header className="-mx-2 flex items-center justify-between py-2" dir="ltr">
                        <BackButton href="/explore/topic-talks" label="بازگشت به فهرست گفتارهای موضوعی" />
                ...
                <section className="mt-5 space-y-2" aria-label={`گفتارهای ${course.title}`}>
                    {Array.from({ length: course.lessonCount }, (_, index) => {
                        ...
                        const href = publishedCourse && lesson ? `/watch/topic-talks/${publishedCourse.id}?lesson=${lesson.id}` : undefined;
                        const status = hasError ? "وضعیت پخش نامشخص است" : isLoading ? "در حال بررسی گفتار…" : href ? "آمادهٔ پخش" : "هنوز منتشر نشده";
                        const content = (
                            <article className="grid min-h-32 grid-cols-[1fr_40%] gap-2">
                                <div className="flex min-w-0 flex-col px-2.5 py-3 text-left">
                                    ...
                                    <p className="mt-1 break-words text-[12px] leading-[17px] text-[#343941] dark:text-slate-100" lang="zh">{topic}</p>
                                ...
                                <div className="my-1.5 mr-1.5 rounded-[8px] bg-[conic-gradient(#eee_25%,#fff_0_50%,#eee_0_75%,#fff_0)] bg-[length:14px_14px] dark:bg-[conic-gradient(#334155_25%,#273344_0_50%,#334155_0_75%,#273344_0)]" role="img" aria-label="تصویر گفتار هنوز اضافه نشده" />
                            </article>
                        );
