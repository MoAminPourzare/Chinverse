    ...
    return (
        <>
            <div className="relative">
                ...
                {message && !open && (
                    <span role="status" aria-live="polite" className="absolute left-0 top-11 z-30 w-52 rounded-lg bg-slate-950 px-3 py-2 text-right text-[11px] font-bold leading-5 text-white shadow-xl" dir="rtl">
                        {message}
                    </span>
                )}
            </div>

            <Dialog open={open} onClose={() => setOpen(false)} className="relative z-[180]" dir="rtl">
                <div className="fixed inset-0 bg-slate-950/45" aria-hidden="true" />
                <div className="fixed inset-0 overflow-y-auto p-4">
                  <div className="flex min-h-full items-end justify-center sm:items-center">
                    <Dialog.Panel as="form" onSubmit={submit} className="w-full max-w-md rounded-lg bg-white p-5 shadow-2xl">
                        <div className="flex items-center justify-between gap-3">
                            <div className="flex items-center gap-2">
                                <ShieldCheck size={20} className="text-[#155aa6]" />
                                <Dialog.Title className="text-base font-black text-slate-900">گزارش محتوا</Dialog.Title>
                            </div>
                        ...
                        {message && <p role="alert" className="mt-3 text-xs font-bold text-rose-600">{message}</p>}
