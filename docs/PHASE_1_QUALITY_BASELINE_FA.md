...
2. تعداد ۲۸ فایل upload قدیمی از قبل در Git track شده‌اند. ignore جدید جلوی
   اضافه‌شدن فایل تازه را می‌گیرد، اما دیسک runtime در Hugging Face Space
   ماندگار نیست و deploy یا restart می‌تواند uploadهای جدید را حذف کند. پیش از
   انتشار عمومی، فایل‌های کاربر باید به object storage منتقل و فایل‌های قدیمی
   Git نیز با تصمیم مالک داده پاک‌سازی شوند.
